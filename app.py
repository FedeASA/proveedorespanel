...
from lib.sheets_ops import (
    ...
    leer_proveedores,
    leer_registros,
    listar_proveedores_garantia,
    obtener_registros_proveedor_sheets,
    marcar_capturados_sheets,
    leer_productos_garantia,
    upsert_registro,
)


# ══════════════════════════════════════════════════════════
#  CONFIGURACIÓN (debe ir antes de leer_ultimo_panel que la usa como default)
# ══════════════════════════════════════════════════════════

SHEET_ID = "1wBkIvtk_KDcPB3Jt1vBcWdVfpRPvzAevX1ooBQ550sI"
DRIVE_ROOT_FOLDER_ID = "1ypABIZBCkvsRED1I8tYE5oxAOT32QTh6"
PRODUCTOS_SHEET_NAME = "RMA ALTAVISTA"


@st.cache_data(ttl=120, show_spinner=False)
def leer_ultimo_panel(sheet_id: str, worksheet_name: str = PRODUCTOS_SHEET_NAME) -> tuple[str, pd.DataFrame]:
    """Lee la hoja específica de productos y retorna su título y registros."""
    ...
    try:
        ...
        return worksheet_name, df.fillna("").astype(str) if not df.empty else df
    except Exception:
        pass
    ...
    for ws in reversed(worksheets):
        ...
        if data:
            df = pd.DataFrame(data)
            return ws.title, df.fillna("").astype(str)

    last_ws = worksheets[-1]
    data = last_ws.get_all_records(default_blank="")
    df = pd.DataFrame(data)
    return last_ws.title, df.fillna("").astype(str) if not df.empty else df
...
st.markdown(
    ...
    <style>
        ...
        /* ══════════════════════════════════════════════════════
           GRID TABLE — columnas con bordes de cuadrícula real
           Aplica a todas las filas que usen st.columns()
        ══════════════════════════════════════════════════════ */

        /* Cada fila horizontal = borde superior + fondo alterno */
        div[data-testid="stHorizontalBlock"] {
            border-left:   1px solid rgba(80, 110, 180, 0.30);
            border-right:  1px solid rgba(80, 110, 180, 0.30);
            border-bottom: 1px solid rgba(80, 110, 180, 0.22);
            margin: 0 !important;
        ...
        div[data-testid="stHorizontalBlock"]
            ...
            border-right: 1px solid rgba(80, 110, 180, 0.22);
        ...
        /* Texto dentro de celdas — alinear verticalmente */
        div[data-testid="stHorizontalBlock"]
            > div[data-testid="stColumn"]
        ...
        div[data-testid="stHorizontalBlock"]:nth-child(even) {
            background: rgba(40, 55, 100, 0.18);
        }
        /* Filas impares: fondo casi transparente */
        div[data-testid="stHorizontalBlock"]:nth-child(odd) {
            background: rgba(20, 25, 50, 0.08);
        }

        /* Hover: highlight de fila */
        div[data-testid="stHorizontalBlock"]:hover {
            background: rgba(60, 90, 160, 0.22) !important;
        }

        /* Borde superior del primer row → forma el borde top de la tabla */
        div[data-testid="stHorizontalBlock"]:first-of-type {
            border-top: 2px solid rgba(80, 110, 180, 0.45);
        }
        ...
        /* ── Row separators (legacy, mantenidos por compatibilidad) ── */
        .row-sep { width:100%; border-radius:1px; margin:1px 0 2px 0; }
        .row-sep-even { height:0px; }
        .row-sep-odd  { height:0px; }

        /* Status-based (Tables 2 & 3) — left border colored */
        .row-sep-nc      { height:0px; }
        .row-sep-cambio  { height:0px; }
        .row-sep-rechazo { height:0px; }
        .row-sep-pending { height:0px; }

        /* ── Resolution badges ── */
        .badge-nc        { background:#28a745; color:#fff; padding:3px 10px;
                           border-radius:4px; font-size:12px; font-weight:600; }
        .badge-cambio    { background:#17a2b8; color:#fff; padding:3px 10px;
                           border-radius:4px; font-size:12px; font-weight:600; }
        .badge-rechazado { background:#dc3545; color:#fff; padding:3px 10px;
                           border-radius:4px; font-size:12px; font-weight:600; }

        /* ── Tabla de garantía — st.dataframe con borde exterior ── */
        div[data-testid="stDataFrame"] {
            border: 1px solid rgba(80, 110, 180, 0.40);
            border-radius: 6px;
            overflow: hidden;
        }
    </style>
...
# NOTA: Las constantes SHEET_ID, DRIVE_ROOT_FOLDER_ID y PRODUCTOS_SHEET_NAME
# fueron movidas arriba del archivo (antes de leer_ultimo_panel) para evitar
# el NameError al usarlas como default value en la firma de la función.
...
def _generar_excels_con_template(
    ...
    """
    Rellena el template con los registros de producto.
